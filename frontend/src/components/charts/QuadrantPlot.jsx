...
export default function QuadrantPlot({ points, height = 280 }) {
  return (
    <div>
      <div className="relative border border-gray-200 rounded-lg" style={{ height }}>
        <div className="absolute inset-0 flex">
          <div className="w-1/2 h-1/2 bg-blue-50/50 border-r border-b border-gray-200 flex items-center justify-center">
            <span className="text-xs text-blue-400 font-medium opacity-60">Well-Liked Underperformers</span>
          </div>
          <div className="w-1/2 h-1/2 bg-green-50/50 border-b border-gray-200 flex items-center justify-center">
        ...
        <div className="absolute bottom-0 left-0 right-0 flex" style={{ height: "50%" }}>
          <div className="w-1/2 bg-red-50/50 border-r border-gray-200 flex items-center justify-center">
        ...
        <div className="absolute left-1 top-1/2 -translate-y-1/2 text-xs text-gray-400 -rotate-90 origin-center whitespace-nowrap">
          ← Negative | Positive →
        </div>
      </div>
        ...
        {Object.entries(ROLE_COLORS)
          ...
          .map(([k, c]) => (
            <div key={k} className="flex items-center gap-1">
              ...
              <span className="text-xs text-gray-500">{ROLE_LABELS[k]}</span>
            </div>
          ))}
      </div>
    </div>
